...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    column(6, actionButton("seed", "Change random seed"))
  ),
  tabPanel(
    "Populate",
    column(
  ...
  tabPanel(
    ...
    column(
      ...
      sliderInput(
        "tauV",
        "Detection parameter (tau), vocalizations",
        0,
        MAXDIS,
        1,
        MAXDIS / 200
      ...
      sliderInput(
        "tauM",
        "Detection parameter (tau), movement",
        0,
        MAXDIS,
        1,
        MAXDIS / 200
      ...
      sliderInput(
        "bpar",
        "Hazard rate parameter (b), vocalizations",
        0,
        MAXDIS,
        1,
        MAXDIS / 200
      ...
      sliderInput(
        "dir_sens",
        "Anisotropy in tau due to direction",
        0,
        2,
        1,
        0.1
...
server <- function(input, output) {
  ...
  a <- reactive({
    ...
    bsims_populate(l(), density = input$D, xy_fun = xy_fun(), margin = margin)
  })
  b <- reactive({
    bsims_animate(
      a(),
  ...
  getset <- reactive({
    ...
    paste0(
      ...
      DURATION,
      ",\n  vocal_rate = ",
      xc(c(input$phi1, input$phi2)),
      ",\n  move_rate = ",
      input$phim,
      ",\n  movement = ",
      input$SDm,
      ",\n  mixture = ",
      xc(c(input$mix, 1 - input$mix)),
      ",\n  allow_overlap = ",
      input$overlap,
      ",\n  initial_location = ",
      input$init_loc,
  ...
  output$plot_est <- renderPlot({
    ...
    op <- par(mfrow = c(1, 3))
    barplot(
      c(True = input$D, Estimate = v$D),
      col = col,
      main = "Density (1/ha)"
    )
    barplot(
      c(True = input$phi1, Estimate = v$phi),
      col = col,
      main = "Cue rate (1/min)"
    )
    barplot(Taus, col = TauCol, main = "Distance parameter (100 m)")
    par(op)
  })
