...
Use functional programming constructs if you do not want to experiment with individual layers
(i.e. do not want state changes and mutations and only care about the outcome):

```{r mag,eval=FALSE}
library(magrittr)

x <- bsims_init(10, 0.5, 1) %>%
  bsims_populate(1) %>%
  bsims_animate(vocal_rate=phi, duration=dur) %>%
  bsims_detect(tau=tau) %>%
  bsims_transcribe(tint=tbr, rint=rbr)
```
