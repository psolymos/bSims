...
3. and mostimportantly to _have fun_ while doing it via an 
...
3. _efficiency_: implementation is computationally efficient
  ustilizing parallel computing backends when available;
...
```{r intro,fig.height=5,fig.width=5}
library(bSims)
...
```{r beh-move,fig.width=10,fig.height=10,out.width='100%'}
...
in the homgeneous habitat case.
...
using `run_app("distfunH")` app for the homogeneous habitat case and
the `run_app("distfunHER")` app for the stratified habitat case.
...
In more complex situations the apps will help identifying
...
Interaction/dependence among layers (observer affecting behaviour,
other individuals/species effects).

Estimation is not part of the package, it was not intended.
