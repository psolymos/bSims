...
The `bsims_animate` function _animates_ the population created by the `bsims_populate` function. `bsims_animate` returns an events objec (with class 'bsims_events').
...
previous call, and also the plot method from the **deldir** package
used for tessellation.

```{r beh-move4,fig.width=6, fig.height=3.5}
...
The `bsims_detect` function _detects_ the events created by the `bsims_animate` function. `bsims_detect` returns a detections objec (with class 'bsims_detections').
...
```{r det,fig.width=3.5, fig.height=3.5}
...
among the strata tha `bsims_detect` function uses a segmented
...
```{r segm,fig.height=4,fig.width=4}
tau <- c(1, 2, 3, 2, 1)
d <- seq(0, 4, 0.01)
...
```{r det2,fig.width=3.5, fig.height=3.5}
e5 <- bsims_animate(
  bsims_populate(
...
that _transcribes_ the detections created by the `bsims_detect` function. `bsims_transcribe` returns a detections objec (with class 'bsims_transcript').
...
endpoints of the the time intervals within the total duration in minutes.
