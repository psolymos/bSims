...
The `extent` of the landscape determines the edge lengths of a quare shaped area.
...
The `edge` argument defines the width of the edge stratum on boths sides of the road.
...
The origin of the cartesian coordinate system inside the landscape is dentered at the middle of the square.
...
```{r landscape-plot,fig.width=6, fig.height=7}
...
If the landscape is stratified, that has no affect on density unless we specify different values
through the `density` argument as a vector of length 3 referring to the HER strata:

```{r pop-3,fig.width=6, fig.height=7}
D <- c(H = 2, E = 0.5, R = 0)
